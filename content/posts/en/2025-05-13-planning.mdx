...
<center>
  <img src="/assets/machine/planning/planning.jpg" className="img-post" />
</center>
...
Nothing interesting from the result above, so we try fuzzing subdomains.
...
Add `grafana.planning.htb` to `/etc/hosts`

# WEB

## CVE-2024-9264

After poking around, we obtain the version of Grafana where there is a CVE that allows remote code execution, use this PoC:
...
We get user root, but there is no flag anywhere; assumption is this is in a **Docker container**.

# USER

So we run linpeas to make it easier.
...
We can see in the following lines there are credentials we can use.
...
Log in via ssh and we will get the user flag.

# ROOT

## Internal Enum

We run linpeas again on user enzo.
...
We see there is `/opt/contabs/crontab.db`, we can view it using **jq**:
...
We get `P4ssw0rdS0pRi0T3c`, but this is not for `su root`.

## Cron

We check the open ports, and there is port 8000 that we can use.

```sh
ssh -L 8000:127.0.0.1:8000 enzo@planning.htb
```

When we visit [http://localhost:8000]([http://localhost:8000), it requires a password; we use the previous password, then use BurpSuite Intruder, and obtain the root username for login.

Apparently this page is used by CronUI, and we can use it to perform vertical privilege escalation.

We create a new schedule and copy */bin/bash* to /tmp.
...
<center>
  <img src="/assets/machine/planning/root.jpg" className="img-post" />
</center>
